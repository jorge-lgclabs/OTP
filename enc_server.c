...
char *encode_message(char *plaintext, char *key);
...
void connection_handler(int connectionSocket, struct sockaddr_in clientAddress) {
    ...
    if (strcmp(confirm_identity, "this is enc_client\n") != 0) {
    ...
    // 2a. Get plaintext
    char *plaintext = get_message(connectionSocket);
    plaintext[strlen(plaintext) - 1] = '\0'; // strip the mandatory line break
    ...
    // 4. Encode plaintext if key is valid
    check_text_and_key(plaintext, key);  // if message and key are valid
    char *ciphertext = encode_message(plaintext, key);

    // 5. Send ciphertext to client
    send_message(ciphertext, connectionSocket);
    printf("SERVER: Ciphertext successfully sent to client\n");
    free(ciphertext);
...
// Receive plaintext and key and return encoded text
char *encode_message(char *plaintext, char *key) {
    size_t len = strlen(plaintext);
    char *encoded_message = calloc((len + 1), 1);
    int message_int;
    int key_int;
    for (int i = 0; i < len; i++) {
        ...
        // Calculate the encoded integer
        int enc_int = (message_int + key_int) % 27;
        // Convert encoded int to char and add it to encoded message
        if (enc_int == 26) {
            encoded_message[i] = ' ';
        }
        else {
            encoded_message[i] = enc_int + 'A';
        }
    }
    encoded_message[len] = '\n';
    free(plaintext);
