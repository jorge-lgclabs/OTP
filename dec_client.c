...
#define IDENTITY "this is dec_client\n"
...
int main(int argc, char *argv[]) {
  ...
  // Check file validity
  check_text_and_key(ciphertext, key);
  ...
  // 2a. Send ciphertext to server
  send_message(ciphertext, socketFD);

  // 2b. Get confirmation of receipt from server
  if (!evaluate_confirmation_message(get_message(socketFD), "2b\n")) {
    error("Server did not receive ciphertext");
  }
  ...
  // 4. Get the plaintext from server
  char *plaintext = get_message(socketFD);
